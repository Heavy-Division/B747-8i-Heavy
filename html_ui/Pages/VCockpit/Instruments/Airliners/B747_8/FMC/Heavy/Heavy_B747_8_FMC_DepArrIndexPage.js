class Heavy_B747_8_FMC_DepArrIndexPage {
	static ShowPage1(fmc) {
		...
		if (origin) {
			...
			fmc.onLeftInput[0] = () => {
				Heavy_B747_8_FMC_DepArrIndexPage.ShowDeparturePage(fmc);
			};
		}
		let rowDestination = [''];
	...
	static ShowDeparturePage(fmc, currentPage = 1) {
						...
						fmc.onRightInput[rowIndex] = () => {
							if (fmc.flightPlanManager.getDepartureProcIndex() === -1) {
								fmc.setOriginRunwayIndex(index, () => {
									fmc.activateRoute();
									Heavy_B747_8_FMC_DepArrIndexPage.ShowDeparturePage(fmc, undefined);
								});
							} else {
		...
		fmc.onPrevPage = () => {
			if (currentPage > 0) {
				Heavy_B747_8_FMC_DepArrIndexPage.ShowDeparturePage(fmc, currentPage - 1);
			}
		};
		fmc.onNextPage = () => {
